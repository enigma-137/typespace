import { GameData, Player, Word, Team } from "./types";
...
export class GameRoom {
  ...
  private difficultyMultiplier: number = 0;
  private intervalId: NodeJS.Timeout | null = null;
  private io: any; // Socket.io instance

  private difficulty: "easy" | "medium" | "hard";

  constructor(id: string, io: any, mode: "ffa" | "coop" = "coop", difficulty: "easy" | "medium" | "hard" = "medium") {
    this.id = id;
    this.io = io;
  ...
  private startLoop() {
    ...
    // Initial words
    for (let i = 0; i < 3; i++) this.spawnWord();
  ...
  private update() {
    ...
    this.state.words = this.state.words.filter(word => {
      ...
      if (word.y > 100) {
        ...
        } else if (this.state.mode === "ffa") {
           // Penalize all players individually
           Object.values(this.state.players).forEach(p => {
               p.score = Math.max(0, p.score - word.points);
           });
        }
    ...
    if (this.state.mode === "coop") {
       ...
       if (this.difficulty === "easy") chance = 0.005; // 0.5% - Very slow
       if (this.difficulty === "hard") chance = 0.05;  // 5% - Aggressive
  ...
  handleInput(playerId: string, wordText: string) {
  ...
  resetGame() {
    ...
    this.state.timeRemaining = 120000;
    this.state.countdown = 0;
    this.state.winner = null;
    this.state.words = [];
    
    // Reset Team Scores
    this.state.teams["human"].score = 0;
    this.state.teams["bot"].score = 0;
